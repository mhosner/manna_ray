...
```bash
test -f ".manna-ray/state.json" && echo "EXISTS" || echo "NEW"
```
...
Initialize state by running:
```bash
export CLAUDE_PROJECT_DIR="$(pwd)"; source ${CLAUDE_PLUGIN_ROOT}/scripts/state.sh && state_init "PROJECT_NAME"
```
